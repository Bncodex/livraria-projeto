...
@Component({
  ...
  imports: [ReactiveFormsModule, RouterLink, ContaConectada],
...
export class Checkout {
  ...
  readonly formulario = this.formBuilder.nonNullable.group({
    ...
    cartao: ['', [Validators.required, Validators.pattern(/^\d{13,19}$/)]],
  ...
  readonly errosFormulario = computed(() => {
    const campos: Record<string, string> = {
      ...
      bairro: 'bairro',
      cidade: 'cidade',
      uf: 'UF com duas letras',
      cep: 'CEP com 8 números',
      titular: 'nome no cartão',
      cartao: 'cartão com 13 a 19 números',
      validade: 'validade no formato MM/AA',
      cvv: 'CVV com 3 ou 4 números',
    };
    return Object.entries(campos)
      .filter(
        ([campo]) =>
          this.formulario.controls[campo as keyof typeof this.formulario.controls].invalid,
      )
      .map(([, nome]) => nome);
  });
  ...
  buscarCep(): void {
    ...
    this.erroCep.set('');
    if (!/^\d{8}$/.test(cep)) {
      this.erroCep.set('Informe um CEP com 8 números.');
      return;
    }
    this.buscandoCep.set(true);
    this.viaCep
